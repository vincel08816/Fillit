...
int		check_piece(int *p, int *m, char board[27][27])
{
	int i; 
	...
	while (i < 8)
	{
		if (ft_isalpha(board[p[i + 1] + m[1]][p[i] + m[0]]))
			return (0);
		i += 2;
	}
	return (1);
...
int		good_piece(int i, char board[27][27], int *p, int *m)
{
	while (!(check_piece(p, m, board)))
	{
		if (g_tet_tab[i][8] + m[0] <= p[27])
			m[0]++;
		else
		{
			if (g_tet_tab[i][9] + m[1] <= p[27])
			{
				m[0] = 0;
				m[1]++;
			}
...
int		ft_solver(int i, int *p, char board[27][27])
{
	...
	while (on)
	{
		if (good_piece(i, board, p, m))
		{
			ft_load(board, i, m, p);
			ret = ft_solver(i + 1, p, board);
		}
		else
			return (0);
		if (!ret)
		{
			...
			if (g_tet_tab[i][8] + m[0] <= p[27])
				m[0]++;
			else if (g_tet_tab[i][9] + m[1] <= p[27])
			{
				m[0] = 0;
				m[1]++;
			}
...
int		ft_fill(int i, int on, int *p, char board[27][27])
{
	p[27] = (p[26] % 2 ? p[26] / 2 + 1 : p[26] / 2);
	//printf("pieces: %d minimum size: %print_board(pieces[27], board);d", p[26], p[27]);
	ft_solver(i, p, board);
	//while (!ft_solver(i, p, board))
	//	p[27]++;
}
